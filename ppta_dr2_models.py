...
class PPTADR2Models(StandardModels):
  ...
  def sw_gp(self, option="default"):

    Tspan = self.params.Tspan
    tmin = self.psr.toas.min()
    tmax = self.psr.toas.max()
    # Mean SW model
    if "bins" in option:
      n_earth_bins = int(option.split('_')[option.split('_').index('bins')+1])  
      n_earth = parameter.Uniform(0,30,size=n_earth_bins)(f'{self.psr.name}_n_earth')
      sw = SW.solar_wind(n_earth=n_earth,n_earth_bins=n_earth_bins,t_init=tmin,t_final=tmax)
      print('Using binned SW model with {} bins'.format(n_earth_bins))
    elif option=="default":
      n_earth = parameter.Uniform(0,30)(f'{self.psr.name}_n_earth')
      sw = SW.solar_wind(n_earth=n_earth,t_init=tmin,t_final=tmax)
      print('Using standard 1/r^2 SW model')
    mean_sw = deterministic_signals.Deterministic(sw, name='mean_sw')

    # SW GP priors
    log10_A_dm_sw = parameter.Uniform(-11., 1.)(self.psr.name + '_log10_A_sw')
    gamma_dm_sw = parameter.Uniform(-2, -1)(self.psr.name + '_gamma_sw')
    ...
    gp_sw = gp_signals.BasisGP(priorFunction=dm_sw_prior, basisFunction=dm_sw_basis, name='sw_gp',combine=self.params.combine)

    return mean_sw + gp_sw
  ...
  def gwb(self,option="hd_vary_gamma"):
    ...
    for option in optsp:
      ...
      if "hd" in option:
        ...
        gwb = pib.create_polarization_signals(b_flags,nfreqs=nfreqs,Tspan=self.params.Tspan,log_A_range=(-18, -6),gamma_range=(0, 7),combine=self.params.combine,gamma_type="common")
