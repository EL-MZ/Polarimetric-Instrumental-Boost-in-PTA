...
class Params(object):
  ...
  def set_default_params(self):
    ...
    if (
        "mcmc_covm_csv" in self.__dict__
        and os.path.isfile(self.mcmc_covm_csv)):
        print("MCMC jump covariance matrix is available")
        self.mcmc_covm = pd.read_csv(
        self.mcmc_covm_csv,
        index_col=0,
      )
    else:
    ...
    if "combine" not in self.__dict__:
        self.combine = True
        print(f"combine set to: {self.combine}")    
  ...
  def init_pulsars(self):
      ...
      if self.array_analysis=='True':
        ...
        if psrs_cache == None:
          ...
          if self.pol_cal == 'True':
            print('Pol calibration is set to True')
            pulsars_list = np.genfromtxt('./analysis/distortion_vectors_DR2/delta_10cm.txt',dtype=str,unpack=True, usecols = 0)
            delta_10cm = np.genfromtxt('./analysis/distortion_vectors_DR2/delta_10cm.txt',dtype=float,unpack=True, usecols = range(1,4))
            delta_20cm = np.genfromtxt('./analysis/distortion_vectors_DR2/delta_20cm.txt',dtype=float,unpack=True, usecols = range(1,4))
            delta_40cm = np.genfromtxt('./analysis/distortion_vectors_DR2/delta_50cm.txt',dtype=float,unpack=True, usecols = range(1,4))
            distr_dict = {}
            for i, psr in enumerate(pulsars_list):
                distr_dict[psr] = {}
                distr_dict[psr]['10CM'] = delta_10cm[:,i]
                distr_dict[psr]['20CM'] = delta_20cm[:,i]
                distr_dict[psr]['40CM'] = delta_40cm[:,i]
                distr_dict[psr]['50CM'] = delta_40cm[:,i]
            # Needs Parallactic-angles path
            attach_deltas(self.psrs, distr_dict, pa_path=f'./analysis/data_dr2_23/',hand=self.hand, project=True)
            print('Adding polynomial distortion vec to pulsars')
...
def init_pta(params_all):
  ...
  for ii, params in params_all.models.items():
    ...
    if params.tm=='default':
      tm = gp_signals.TimingModel(use_svd=bool(params.tm_svd),combine=params.combine)
    elif params.tm=='fast':
      ...
      tm = gp_signals.BasisGP(prior, basis, name='ridge',combine=params.combine)
    ...
    if 'noisefiles' in params.__dict__.keys():
      ...
      print('Using noise parameters from the file: ', noisedict)
...
# def attach_deltas(psrs, global_delta):
#     """
#     Loop over a list of enterprise.Pulsar objects and attach
#     backend-specific Δ-vectors.
#     """
#     for psr in psrs:
#         # grab the list of backends for this pulsar (or empty list)
#         backends = psr.flags.get("B", [])
#         # collect the Δ-vectors, with a clear error if something’s missing
#         try:
#             deltas = [global_delta[psr.name][bk] for bk in backends]
#         except KeyError as e:
#             raise KeyError(f"Delta for pulsar '{psr.name}', backend '{e.args[0]}' not found") from None

#         # stack into a 2D array (or create an empty array if no backends)
#         psr.distort_vect = (
#             np.vstack(deltas)
#             if deltas
...
def attach_deltas(psrs, global_delta,hand='right', pa_path=None, project=False):
    ...
    for psr in psrs:
        ...
        if not deltas:
            # create an empty array with 3 columns assuming 3D vectors
            psr.distort_vect = np.empty((0, 3)) 
            continue

        # stack into a 2D array of shape (N_toas, 3)
        delta_array = np.vstack(deltas)
        
        # --- Apply the Projection Algebra Directly if requested ---
        if project:            
            ...
            # Construct the trigonometric arrays (factor of 2 for spin-2 field)
            cos_2t = np.cos(2 * theta)
            sin_2t = np.sin(2 * theta)

            # Extract the unprojected components
            dx = delta_array[:, 0]
            dy = delta_array[:, 1]
            dz = delta_array[:, 2]
            ...
            delta_array = np.column_stack((dx_proj, dy_proj, dz_proj))

        # Attach the final (projected or unprojected) vectors
        psr.distort_vect = delta_array
